...
class Frame(Element):
    ...
    def getGridElement(self, pos):
        if slave := self.widget.grid_slaves(column=pos.x, row=pos.y):
            return slave[0].element

    def getGridSize(self):
        size = self.widget.grid_size()
    ...
    def setGridElement(self, elementClass, column, row, **parameters):
        element = elementClass(self.parentPage, column=column, row=row, padx=5, sticky="NSEW", relief="groove", bg="gray85", **parameters)
